...
enum OrderStatus:Int {
    ...
    case active = 2
    case waitingReview = 3
    case completed = 4
    case decline = 5
}

class MySpotOrderWaiting {
    ...
    static func with(json: JSON) -> MySpotOrderWaiting {
        
        let order = MySpotOrderWaiting()
    ...
    static func with(jsons: [JSON]) -> [MySpotOrderWaiting] {
        var orderArray = [MySpotOrderWaiting]()
        
        for json in jsons {
            let order = MySpotOrderWaiting.with(json: json)
